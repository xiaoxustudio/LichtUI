    ...
    <div :class="bem.b()">
        <div :class="bem.e('wrapper')">
            <TransitionGroup name="fade" tag="div">
                <Msg v-for="item, index in queue.value" :key="item.key" :index="index" :uuid="item.uuid"
                    :content="item.content" :type="item.type" :duration="item.duration" :closed="item.closed" :style="{
                        top: `${index * 48 + 16}px`
                    }" />
            </TransitionGroup>
        </div>
